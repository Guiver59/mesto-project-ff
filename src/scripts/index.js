...
import {loadUserInfo, loadCards, saveUserInfo, addCard, updateAvatar} from './../components/api.js'
...
function handleProfileFormSubmit(evt) {
    ...
    const promisSaveUserInfo = new Promise(function(resolve,reject){
      saveUserInfo({ name: newName, about: newDesc}).then((res)=>{
        //return res        
        resolve(res);
      }) 
      .catch((err)=>{
    ...
    promisSaveUserInfo.then((newUserData)=>
    {
...
function handleFormAdd(evt) {
    ...
    new Promise((resolve,reject)=>{      
      ...
      const newCard = createCard(newCardData, removeCard, reactCard, openCard, userInfo._id, popupDelete);
...
function handleEditAvatar(evt) {  
  ...
  new Promise((resolve,reject)=>{
    updateAvatar(newAvatarUrl).then((userInfo)=>{resolve(userInfo)}).catch((err)=>{console.log(err)});
  }).then((newUserInfo)=>{
...
function setCards(initialCards){
  initialCards.forEach((item) => {
    const newCard = createCard(item, removeCard, reactCard, openCard, userInfo._id, popupDelete);
...
Promise.all([promiseLoadUser, promiseLoadCards])
  .then((data) => {
    //console.log(value);
